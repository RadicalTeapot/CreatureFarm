...
class Ui(object):
    ...
    def build(self):
        ...
        self.creature_panel.set_layout(1)

        self.right_panel = Panel(
            Settings.WIDTH - 250, Settings.HEIGHT - 450, 250, 450
        )
        self.right_panel.set_layout(1)

        self.right_panel.add_label(AttributeLabel(self.game, 'name'))
        self.right_panel.add_label(AttributeLabel(self.game, 'hp', pre='HP:'))
        self.right_panel.add_label(
            AttributeLabel(self.game, 'strength', pre='Strength:')
        )
        self.right_panel.add_label(
            AttributeLabel(self.game, 'agility', pre='Agility:')
        )
        self.right_panel.add_label(
            AttributeLabel(self.game, 'stamina', pre='Stamina:')
        )
        self.right_panel.add_label(
            AttributeLabel(self.game, 'speed', pre='Speed:')
        )
        self.right_panel.add_label(
            AttributeLabel(self.game, 'hunger', pre='Hunger:')
        )
        self.right_panel.add_label(
            AttributeLabel(self.game, 'tired', pre='Sleep:')
        )

        self.right_panel.show()

        self.bottom_panel = Panel(0, 0, Settings.WIDTH, 150)
        self.bottom_panel.set_layout(0)

        button = Button('(c) Creatures')
        button.register_handler(self.game.show_creatures)
        self.bottom_panel.add_button(button)
